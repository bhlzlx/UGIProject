...
namespace ugi {
    ...
    void SignedDistanceFieldImage2D(
        ...
        SDFFlag* flagMap = new SDFFlag[dstWid*dstHei];
        constexpr int searchDistance = 16;
            ...
            for( int32_t j = 0; j<dstWid; ++j) {
                ...
                if(flag.inner) {
                    if( flag.opaque ) { // 不透明的直接算距离
                        ...
                        distance = (float)src[srcX+srcWid*srcY] / 255.0f;
                    }
    ...
    Glyph GetGlyphData( uint16_t charCode ) {
        ...
        //
        SignedDistanceFieldImage2D( GlyphBitmapPool, c.width,c.height, GlyphSDFPool, c.width, c.height );
        //
        return c;
    }
    ...
    void TPTest::tick() {
        ...
        cmdbuf->beginEncode(); {
            ...
            _uniformAllocator->allocateForDescriptor(_uniformDescriptor, UBOData);
            //
