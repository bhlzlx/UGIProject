...
namespace ugi {
    ...
    bool HelloWorld::initialize( void* _wnd, hgl::assets::AssetsSource* assetsSource ) {
        ...
        m_vertexBuffer = m_device->createBuffer( BufferType::VertexBuffer, sizeof(float) * 5 * 3 );
        Buffer* vertexStagingBuffer = m_device->createBuffer( BufferType::StagingBuffer, sizeof(float) * 5 * 3 );
        float vertexData[] = {
            -0.5, -0.5, 0, 0, 0,
            0, 0.5, 0, 0.5, 1.0,
            0.5, -0.5, 0, 1.0, 0
        ...
        m_indexBuffer = m_device->createBuffer( BufferType::IndexBuffer, sizeof(indexData));
        Buffer* indexStagingBuffer = m_device->createBuffer( BufferType::StagingBuffer, sizeof(indexData) );
    ...
    void HelloWorld::tick() {
        ...
        cmdbuf->beginEncode(); {
            ...
            auto ubo = m_uniformAllocator->allocate(sizeof(argMat));
