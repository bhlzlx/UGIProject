...
namespace ugi {
    ...
    Texture* CreateTextureDDS(Device* device, uint8_t const* data, uint32_t dataLen) {
		...
		for(uint32_t i = 0; i<file.GetMipCount(); ++i) {
			...
			region.offset = {0, 0, 0};
			region.extent = {desc.width, desc.height, 1};
		}
		texture->updateRegions(device, regions.data(), regions.size(), (uint8_t const*)pixelContent, pixelContentSize, offsets.data(), )
